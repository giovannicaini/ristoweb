...
namespace UserFrosting\Sprinkle\Risto\Controller;
...
use UserFrosting\Sprinkle\Risto\Database\Models\RistoCategoria;
use UserFrosting\Sprinkle\Risto\Database\Models\RistoComanda;
use UserFrosting\Sprinkle\Risto\Database\Models\RistoComandaDettaglio;
use UserFrosting\Sprinkle\Risto\Database\Models\RistoConto;
use UserFrosting\Sprinkle\Risto\Database\Models\RistoEvento;
use UserFrosting\Sprinkle\Risto\Database\Models\RistoPostazione;
use UserFrosting\Sprinkle\Risto\Database\Models\RistoProdotto;
use UserFrosting\Sprinkle\Risto\Database\Models\RistoCassa;
use UserFrosting\Sprinkle\Risto\Database\Models\RistoStampante;
...
class RistoController extends SimpleController {
	
	private function eventoCorrente(){
		return RistoEvento::where('attivo',true)->first();
	}
	
	private function cassaCorrente(){
		$cassa = RistoCassa::find($_SESSION["cassa_id"]);
		if ($cassa)
			return $cassa;
		else
			return false;
	...
	public function pageComande(Request $request, Response $response, array $args): Response {
		...
		//$comande = RistoComanda::orderBy('n_ordine','DESC')->where('evento_id',$this->eventoCorrente()->id)->where('created_at','>=',"2022-09-25")->get();
		$comande = RistoComanda::orderBy('n_ordine','DESC')->where('evento_id',$this->eventoCorrente()->id)->get();
	...
	public function pageComande2(Request $request, Response $response, array $args)	 {
	...
	public function pageComanda(Request $request, Response $response, array $args): Response {
		$params = $request->getQueryParams();
        $currentUser = $this->ci->currentUser;
		$comanda = RistoComanda::find($args["comanda_id"]);
		$cassiere = User::find($comanda->cassiere_id);
		$cassa = RistoCassa::find($comanda->cassa_id);
		if (!$comanda)
			throw new NotFoundException();
		
		$categorie1 = RistoCategoria::where('evento_id',$comanda->evento_id)->where('colonna',1)->orderBy('ordine')->get();
		foreach ($categorie1 as $categoria){
			$prodotti = RistoProdotto::where('categoria_id',$categoria->id)->where('attivo',true)->orderBy('ordine')->get();
			foreach ($prodotti as $prodotto){
				$dettaglio = RistoComandaDettaglio::where('comanda_id',$comanda->id)->where('prodotto_id',$prodotto->id)->first();
				if ($dettaglio){
		...
		$categorie2 = RistoCategoria::where('evento_id',$comanda->evento_id)->where('colonna',2)->orderBy('ordine')->get();
		...
		$dettagli = RistoComandaDettaglio::where('comanda_id',$comanda->id)->get();
		foreach($dettagli as $dettaglio){
			$dettaglio->setAttribute('prodotto',RistoProdotto::find($dettaglio->prodotto_id));
		}
		
		$conti = RistoConto::where('evento_id',$this->eventoCorrente()->id)->orderBy('nome')->get();
	...
	public function apiEliminaComanda($request, $response, $args) {
		...
		Capsule::transaction(function () use ($comanda, $currentUser) {
			...
			$comanda_dettaglio = RistoComandaDettaglio::where('comanda_id',$comanda->id)->get();
	...
	public function apiCambiaEventoCorrente($request, $response, $args) {
		...
		$evento = RistoEvento::find($params["evento"]);
		if (!$evento){
	...
	public function apiCambiaCassaCorrente($request, $response, $args) {
		...
		$cassa = RistoCassa::find($params["cassa"]);
		if (!$cassa){
	...
	public function apiInviaMessaggioCassa($request, $response, $args) {
		...
		$stampante = RistoStampante::find($cassa->stampante_id);
	...
	public function apiNuovaComanda($request, $response, $args) {
		...
		$evento = RistoEvento::find($params["evento_id"]);
		if (!$evento){
			$this->ci->alerts->addMessageTranslated('danger', 'Evento non trovato!');
			return $response->withJson([], 503);
		}
		
		$cassa = RistoCassa::find($params["cassa_id"]);
		if (!$cassa){
			$this->ci->alerts->addMessageTranslated('danger', 'Cassa non trovatao!');
			return $response->withJson([], 503);
		}
		...
		$last_comanda = RistoComanda::where('evento_id',$evento->id)->orderBy('n_ordine','DESC')->first();
		if (!$last_comanda)
			$n_ordine = 1;
		else
	...
	public function apiCompilaComanda($request, $response, $args) {
		...
		foreach ($ordini as $ordine){
			$prodotto = RistoProdotto::find($ordine->prodotto_id);
			if (!$prodotto){
		...
		Capsule::transaction(function () use ($comanda, $ordini, $currentUser) {
			...
			$dettagli_old = RistoComandaDettaglio::where('comanda_id',$comanda->id)->get();
			...
			foreach ($ordini as $ordine){
				$prodotto = RistoProdotto::find($ordine->prodotto_id);
				$dettaglio = new RistoComandaDettaglio();
	...
	public function apiInviaComanda($request, $response, $args) {
		...
		Capsule::transaction(function () use ($comanda, $sconto, $buoni, $pagato, $conto, $su_conto, $currentUser) {
			...
			if ($conto){
				$conto_ok = RistoConto::where('id',$conto)->where('evento_id',$this->eventoCorrente()->id)->first();
				
				if (!$conto_ok){
					$conto_ok = new RistoConto();
					$conto_ok->nome = $conto;
	...
	public function saveDbToJson(Request $request, Response $response, array $args){
		$json = [];
		$json['casse'] = RistoCassa::get();
		$json['categorie'] = RistoCategoria::get();
		$json['comande'] = RistoComanda::get();
		$json['comande_dettagli'] = RistoComandaDettaglio::get();
		$json['conti'] = RistoConto::get();
		$json['eventi'] = RistoEvento::get();
		$json['postazioni'] = RistoPostazione::get();
		$json['prodotti'] = RistoProdotto::get();
		$json['stampanti'] = RistoStampante::get();
		$text = json_encode($json);
		$filename = "risto_db_".date("Y-m-dTH:i:s");
	...
	public function pdfComanda(Request $request, Response $response, array $args) {
		...
		foreach($dettagli as $dettaglio){
			$dettaglio->setAttribute('prodotto',RistoProdotto::find($dettaglio->prodotto_id));
			$postazione = RistoCategoria::find($dettaglio->prodotto->categoria_id)->postazione_id;
			if (!in_array($postazione, $postazioni_id))
				array_push($postazioni_id, $postazione);
		}
		
		$postazioni_scontrino = RistoPostazione::whereIn('id',$postazioni_id)->where('accoda_a_scontrino',true)->where('evento_id',$comanda->evento_id)->orderBy('ordine')->get();
		foreach($postazioni_scontrino as $p){
			$dettaglip = RistoComandaDettaglio::where('comanda_id',$comanda->id)->whereIn('prodotto_id',RistoProdotto::whereIn('categoria_id',RistoCategoria::where('postazione_id',$p->id)->get()->pluck('id')->toArray())->get()->pluck('id')->toArray())->get();
			foreach($dettaglip as $dettaglio){
				$dettaglio->setAttribute('prodotto',RistoProdotto::find($dettaglio->prodotto_id));
			}
			$p->setAttribute('dettagli',$dettaglip);
		}
		$postazioni_no_scontrino = RistoPostazione::whereIn('id',$postazioni_id)->where('accoda_a_scontrino',null)->where('evento_id',$comanda->evento_id)->orderBy('ordine')->get();
	...
	public function pdfLista(Request $request, Response $response, array $args) {
		...
		$categorie = RistoCategoria::orderBy('ordine')->where('evento_id',$this->eventoCorrente()->id)->get();
		foreach ($categorie as $c){
			$prodotti = RistoProdotto::where('categoria_id',$c->id)->orderBy('ordine')->get();
	...
	public function pdfRiepilogo(Request $request, Response $response, array $args) {
		...
		$categorie = RistoCategoria::where('evento_id',$this->eventoCorrente()->id)->get();
		foreach ($categorie as $c){
			$prodotti = RistoProdotto::where('categoria_id',$c->id)->orderBy('ordine')->get();
			foreach($prodotti as $p){
				$conta = 0;
				$incasso = 0;
				$comande_dettagli = RistoComandaDettaglio::where('prodotto_id',$p->id)->get();
				foreach ($comande_dettagli as $cd){
					$conta += $cd->quantita;
					$comanda = RistoComanda::find($cd->comanda_id);
					if ($comanda){
	...
	public function pdfRiepilogoGenerale(Request $request, Response $response, array $args) {
		...
		$eventi_id = [5,6,7,8];
		$eventi = RistoEvento::whereIn('id',$eventi_id)->get();
		//$eventi = RistoEvento::where('nome',$nome_evento)->get();
		...
		foreach ($eventi as $evento){
			...
			$incasso = 0;
			$comande_pagate = RistoComanda::where('evento_id',$evento->id)->get();
		...
		$categorie = RistoCategoria::whereIn('evento_id',$eventi_id)->orderBy('ordine')->get();
		foreach($categorie as $c){
			if (!array_key_exists($c->nome,$cate_array)){
				$cate_array[$c->nome] = RistoCategoria::whereIn('evento_id',$eventi_id)->where('nome',$c->nome)->get()->pluck('id')->toArray();
			}
		}
		...
		foreach ($cate_array as $categoria_nome => $categorie_id){
			$prod_array = Array();
			$prodotti = RistoProdotto::whereIn('categoria_id',$categorie_id)->get();
			foreach($prodotti as $p){
				if (!array_key_exists($p->nome,$prod_array)){
					$prod_array[$p->nome] = RistoProdotto::whereIn('categoria_id',$categorie_id)->where('nome',$p->nome)->get()->pluck('id')->toArray();
				}
			}
		...
		foreach ($cate_array2 as $categoria_nome => $prod_array){
			...
			foreach($prod_array as $nome_prodotto => $ids_prodotti){
				$eventi2 = RistoEvento::whereIn('id',$eventi_id)->get();
				foreach($eventi2 as $e){
					$conta = 0;
					$incasso = 0;
					$p = RistoProdotto::whereIn('id',$ids_prodotti)->first();
					$comande_dettagli = RistoComandaDettaglio::whereIn('prodotto_id',$ids_prodotti)->whereIn('comanda_id', RistoComanda::where('evento_id',$e->id)->get()->pluck('id')->toArray())->get();
					foreach ($comande_dettagli as $cd){
						$conta += $cd->quantita;
						$comanda = RistoComanda::find($cd->comanda_id);
						if ($comanda){
...
private function compactCharTable($printer, $start = 4, $header = false)
{
	...
	private function printScontrino($comanda, $dettagli, $postazioni_scontrino, $stampa_cassa_attiva = false){
		$evento = RistoEvento::find($comanda->evento_id);
		$cassiere = User::find($comanda->cassiere_id);
		$cassa = RistoCassa::find($comanda->cassa_id);
		if (!$comanda)
			throw new NotFoundException();
		...
		if ($stampa_cassa_attiva){
			$cassa_corrente = $this->cassaCorrente();
			$stampante = RistoStampante::find($cassa_corrente->stampante_id);
		}
		$connector = new NetworkPrintConnector($stampante->ip, 9100);
		try {
			$printer = new Printer($connector);	
	...
	private function printPostazioni($comanda, $dettagli, $postazioni_no_scontrino, $stampa_cassa_attiva = false){
		...
		foreach($postazioni_no_scontrino as $p){
			if ($p->stampante_id)
				$stampante = RistoStampante::find($p->stampante_id);
			else
				$stampante = RistoStampante::find($cassa->stampante_id);
			if ($stampa_cassa_attiva){
				$cassa_corrente = $this->cassaCorrente();
...
private function printPostazione($comanda, $postazione, $printer){
	...
	if ($postazione->stampa_coperti){
		$coperti = RistoComandaDettaglio::where('comanda_id',$comanda->id)->whereIn('prodotto_id',RistoProdotto::where('nome',"Coperto")->get()->pluck('id'))->first()->quantita;
